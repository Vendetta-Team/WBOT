    ...
    if (log[channel.guild.id].cdc) {
        hastebin(JSON.stringify(channel.permissions), { url: "https://pastie.io/",extension: 'txt' }).then(async (haste) => {
            ...
            const embed = new Discord.MessageEmbed()
                .setTitle('채널 삭제 확인됨')
                .addField('채널 삭제자', user)
                .addField('채널 삭제자 유저 태그', user.tag)
                .addField('채널 삭제자 유저 아이디', userid)
                .addField('채널 이름', channel.name)
                .addField('채널 권한', haste)
                .addField('채널 아이디', channel.id)
            if (client.channels.cache.get(log[channel.guild.id].cdc)) {
                client.channels.cache.get(log[channel.guild.id].cdc).send(embed)
            }
