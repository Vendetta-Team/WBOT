        ...
        if (log[role.guild.id].rcc) {
            hastebin(JSON.stringify(role.permissions), { url: "https://pastie.io/",extension: 'txt' }).then(async (haste) => {
                ...
                const embed = new Discord.MessageEmbed()
                    .setTitle('역할 생성 확인됨')
                    .addField('역할 제작자', user)
                    .addField('역할 제작자 유저 태그', user.tag)
                    .addField('역할 제작자 DUID', userid)
                ...
                if (client.channels.cache.get(log[role.guild.id].rcc)) {
                    client.channels.cache.get(log[role.guild.id].rcc).send(embed)
                }
