module.exports = {
    name: ["관리자제거", "ㄱㄹㅈㅈㄱ", "delmanager"],
    description: "봇에 대한 관리자 권한을 제거합니다.",
    type: "manager",
    usage: "!관리자제거 @유저#1234",
    async run(client, message, args, db, config) {
        try {
            ...
            if (!member) return message.reply("봇의 관리자 권한을 제거할 유저를 언급해주세요.");
            if (!managers.includes(member.user.id)) return message.reply("봇의 관리자 권한이 없는 유저입니다.");
            await db.pull(`managers.${member.user.id}`, member.user.id);
